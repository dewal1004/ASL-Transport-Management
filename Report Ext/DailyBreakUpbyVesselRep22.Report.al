report 80108 "Daily BreakUp by Vessel Rep 22"
{
    DefaultLayout = RDLC;
    RDLCLayout = './DailyBreakUpbyVesselRep22.rdlc';
    ...
    dataset
    {
        dataitem("Inventory Posting G Cat Tot"; "Inventory Posting Group")
        {
            ...
            begin
                ...
                if locate2.FindFirst then
                    repeat
                        ...
                        Operatn.SetFilter(Operatn."Starting Date", '<=%', StartingDate);
                        Operatn.SetRange(Operatn.Vessel, locate2.Code);
                        if Operatn.FindSet then begin
                            locate2.VSDVal := StartingDate - Operatn."Starting Date";
                            if locate2.VSDVal < 0 then
                                locate2.VSDVal := locate2.VSDVal * (-1);
                        end;
                        locate2.Modify();
                ...
                locate.SetRange(locate."Catch Date", StartingDate);
                TotLoc := locate.Count;
            ...
            begin
                ...
                if locate2.Find('-') then
                    repeat
                        ...
                        locate2."Last Rep Operation No. Val" :=
                           locate2.LastRepOpNo(locate2.Code, locate2.GetFilter("date filter"));
                        //locate2.CALCFIELDS(locate2."Voyage Sea Any Time");
                        locate2.VSDVal := locate2.VoySeaAnyTime(locate2."Last Rep Operation No.", locate2.Code,
                          locate2.GetFilter("date filter"));
                        //Calculate Vessel Point
                        if Operatn.Get(locate2."Last Rep Operation No. Val") then
                            Operatn.SetFilter(Operatn."Posting Date Filter", '%1', GetRangeMax("Date Filter"));
                ...
                FishCounter := 0;
                DateFilter := "Inventory Posting G Cat Tot".GetFilter("Date Filter");
                Evaluate(StartingDate, DateFilter);
        ...
        dataitem("Inventory Posting Group"; "Inventory Posting Group")
        {
            ...
            begin
                ...
                locate.SetFilter(locate.ETD, '>=%1', StartingDate);
                if (StartingDate >= locate.ETD) and (StartingDate <= locate.ETA) then
                    if locate.FindFirst then
                        repeat
                            //IF locate.VSDVal<>0 THEN
                            //BEGIN
                ...
                REPEAT
                  ...
                  locate2."Last Rep Operation No. Val":=
                    locate2.LastRepOpNo(locate2.Code,locate2.GETFILTER("date filter"));
                  //locate2.CALCFIELDS(locate2."Voyage Sea Any Time");
                  locate2.VSDVal:= locate2.VoySeaAnyTime(LastRepOptNo,locate2.Code,
                    locate2.GETFILTER("date filter"));
                  locate2.MODIFY();
                UNTIL locate2.NEXT=0;
