report 97150 "Consolidated Voyage P&L3"
{
    // "Job Catch Default"."No."
    DefaultLayout = RDLC;
    RDLCLayout = './ConsolidatedVoyagePL3.rdlc';
    ...
    dataset
    {
        dataitem(Jobss; Job)
        {
            DataItemTableView = SORTING (Vessel, "Starting Date") WHERE ("Voyage Ended" = CONST (true));
        ...
        dataitem("Sea Food categories"; "Sea Food categories")
        {
            ...
            dataitem("Job Ledger Entry Rep"; "Job Ledger Entry")
            {
                ...
                DataItemTableView = SORTING ("Location Code", GroupSort) WHERE ("Location Code" = CONST ('CRM-ASL'), "Gen. Prod. Posting Group" = FILTER ('FIS' | ''));
            ...
            dataitem("Job catch Default"; "Job catch Default")
            {
                ...
                column(Job_catch_Default__No___Control1000000008; "No.")
                {
                }
                column(No_B__Control1000000009; "No.B")
                {
                }
                column(NairaValB_Control1000000010; NairaValB)
                {
                }
                column(PrdPrcB_Control1000000011; PrdPrcB)
                {
                }
                column(QtyB_Control1000000012; QtyB)
                {
                }
                column(Job_catch_Default_GroupSort_Control1000000013; GroupSort)
                {
                }
        ...
        {
            DataItemTableView = SORTING ("Expected Cost", "Gen. Prod. Posting Group") WHERE ("Gen. Prod. Posting Group" = FILTER (<> 'FIS'));
