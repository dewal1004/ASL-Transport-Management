report 53176 "New Daily Points by Capt Repv"
{
    DefaultLayout = RDLC;
    RDLCLayout = './NewDailyPointsbyCaptRepv.rdlc';
    ...
    dataset
    {
        dataitem("Integer"; "Integer")
        {
            ...
            begin
                ...
                if RepDate <> 0D then begin
                    ...
                    JobsPointVal.SetFilter(JobsPointVal."Posting Date Filter", '%1..%2', 19800101D, Today);
        ...
        dataitem(VoyMedian; Job)
        {
            ...
            begin
                ...
                if Loc.Get(VoyMedian.Vessel) then Desc := Loc.Name else Desc := '';
                if Resource.Get(VoyMedian."Person Responsible") then Skipper := Resource.Name else Skipper := VoyMedian."Person Responsible";
            end;
        }
    }
