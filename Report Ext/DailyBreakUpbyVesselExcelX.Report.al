report 80083 "Daily BreakUp by Vessel ExcelX"
{
    DefaultLayout = RDLC;
    RDLCLayout = './DailyBreakUpbyVesselExcelX.rdlc';

    dataset
    {
        dataitem("Inventory Posting Group"; "Inventory Posting Group")
        {
            DataItemTableView = SORTING ("Statistics Group", Category);
            ...
            begin
                ...
                //locate.SETRANGE(locate."Location Group",LocGroup);
                TotLoc := locate.Count;

                //locate.CALCFIELDS(locate."Sea Days");
                ...
                if locate.Find('-') then
                    repeat
                        /**///check if there is a catch for the day then assign
                            //No Catch Record Exist
                        job1.SetFilter(job1."Posting Date Filter", '=%1', locate."date filter");
                        // job1.SETRANGE(job1."Type Filter",0);   #1
                        job1.SetRange(job1."Location Filter", locate.Code);
